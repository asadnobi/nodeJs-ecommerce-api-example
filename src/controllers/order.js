...
exports.order_get_all_data = async (req, res, next) => {
    try {
        ...
        if (response.lastPage == 0) return res.status(204).send({status: false, msg: "Maybe Order was not found!"});
...
exports.order_post_data = async (req, res, next) => {
    ...
    try {
        ...
        if(!result) return res.status(204).send({status: false, msg: "Maybe Order was not found!"});
...
exports.order_get_data = async (req, res, next) => {
    ...
    if(!mongoose.Types.ObjectId.isValid(_id)) {
        return res.status(400).send({status: false, msg: "Could not get Order with id=" + _id});
    }
    try {
...
exports.order_update_data = async (req, res, next) => {
    const _id = req.params.orderId;
    if(!mongoose.Types.ObjectId.isValid(_id)) {
        return res.status(400).send({status: false, msg: "Could not update Order with id=" + _id});
    }
    try {
...
exports.order_delete_data = async(req, res, next) => {
    const _id = req.params.orderId;
    if(!mongoose.Types.ObjectId.isValid(_id)) {
        return res.status(400).send({status: false, msg: "Could not delete Order with id=" + _id});
    }
    try {
