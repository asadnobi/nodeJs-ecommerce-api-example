...
const getAllApiKey = async (req, res, next) => {
  try {
      ...
      if (response.lastPage == 0) return res.status(400).send({status: false, msg: "Maybe Key was not found!"});
      if (response.page > response.lastPage) return res.status(400).send({status: false, msg: `You've reached the last page, LAST PAGE: ${response.lastPage}`});
...
const postAnApiKey = async (req, res, next) => {
    ...
    if(foundApiKey) return res.status(400).send({status: false, msg: "Existing user found. Already provide you a API key"});
...
const getAnApiKey = async (req, res, next) => {
  ...
  if(!mongoose.Types.ObjectId.isValid(_id)) {
      return res.status(500).send({status: false, msg: "Could not get Key with id=" + _id});
  }
  try {
      const result = await ApiKeyModel.findById(_id)
      .select('-createdAt -updatedAt -__v');
      if(!result) return res.status(404).send({status: false, msg: "Maybe Key was not found!"});
...
const updateAnApiKey = async (req, res, next) => {
  const _id = req.params.keyId;
  if(!mongoose.Types.ObjectId.isValid(_id)) {
      return res.status(500).send({status: false, msg: "Could not update Key with id=" + _id});
  }
  try {
...
const deleteAnApiKey = async(req, res, next) => {
  const _id = req.params.keyId;
  if(!mongoose.Types.ObjectId.isValid(_id)) {
      return res.status(500).send({status: false, msg: "Could not delete Key with id=" + _id});
  }
  try {
...
const deleteAllApiKey = async(req, res, next) => {
  try {
      ...
      if(!result) return res.status(404).send({status: false, msg: "Opps! Something went wrong"});
